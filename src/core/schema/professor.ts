...
export const ProfessorInfoSchema = z.object({
  firstNameTh: z.string().trim().min(1, "กรุณากรอกชื่อ (ภาษาไทย)").regex(/^[ก-๙\s]+$/, "กรุณากรอกเป็นภาษาไทยเท่านั้น"),
  lastNameTh: z.string().trim().min(1, "กรุณากรอกนามสกุล (ภาษาไทย)").regex(/^[ก-๙\s]+$/, "กรุณากรอกเป็นภาษาไทยเท่านั้น"),
  firstNameEn: z.string().trim().min(1, "กรุณากรอกชื่อ (ภาษาอังกฤษ)").regex(/^[a-zA-Z\s]+$/, "กรุณากรอกเป็นภาษาอังกฤษเท่านั้น"),
  lastNameEn: z.string().trim().min(1, "กรุณากรอกนามสกุล (ภาษาอังกฤษ)").regex(/^[a-zA-Z\s]+$/, "กรุณากรอกเป็นภาษาอังกฤษเท่านั้น"),
  phone: z.string().trim().regex(/^0[0-9]{8,9}$/, "เบอร์โทรต้องเป็นตัวเลข 9-10 หลัก และขึ้นต้นด้วย 0"),
  email: z.string().trim().lowercase().email("รูปแบบอีเมลไม่ถูกต้อง"),
