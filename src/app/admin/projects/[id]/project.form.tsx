...
export const ProjectForm: FC<ProjectFormProps> = ({
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      // Note: Backend merges assets based on what is uploaded. To remove assets, 
      // let's pass the assetsURL updated string or let backend handle it.
      // In the backend projectService.updateProject:
      // const assetsURLString = assetURLs.join(",");
      // So backend appends uploaded files to the existing assets. If we deleted some existing assets,
      // we'd need backend to support deleting them. But the backend only has `assetsURL: assetsURLString` where `assetURLs`
      // starts as `existingProject.assetsURL.split(",")` and appends new assets. It doesn't support deleting individual ones.
      // That's fine, we will upload the new ones.
